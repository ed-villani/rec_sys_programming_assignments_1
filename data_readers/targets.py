    ...
    def solve(self, item_dict: ItemDict, user_dict: UserDict, avg: int):
        ...
        for index, data in enumerate(np.array(self.data)):
            ...
            if user in user_dict() and item in item_dict():
                user_rates = user_dict.get_rates(user)
                div, user_item_similarities = item_dict.get_similarity(item, user_rates)
                user_rates_values = np.array([value['normal'] for value in user_rates.values()])

                item_med = item_dict.get_avg(item)

                if div > 0.0001:
                    self._solution[index] = item_med + np.dot(user_item_similarities, user_rates_values) / div
                else:
